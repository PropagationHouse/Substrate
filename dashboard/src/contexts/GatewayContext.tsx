...
function loadConfig() {
  try { return JSON.parse(localStorage.getItem('substrate-dashboard-config') || '{}'); } catch { return {}; }
}
function saveConfig(url: string, token: string) {
  localStorage.setItem('substrate-dashboard-config', JSON.stringify({ url, token }));
}
