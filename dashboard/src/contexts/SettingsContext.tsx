...
const FONT_REFRESH_STORAGE_KEY = 'substrate:font-refresh-20260312';
...
function resolveInitialFont(): FontName {
  const saved = localStorage.getItem('substrate:font');
  ...
  if (!hasRefreshedFont) {
    ...
    if (shouldAdoptInstrumentSans) {
      localStorage.setItem('substrate:font', 'instrument-sans');
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  const [soundEnabled, setSoundEnabled] = useState(localStorage.getItem('substrate:sound') === 'true');
  const [ttsProvider, setTtsProvider] = useState<TTSProvider>(() => migrateTTSProvider(localStorage.getItem('substrate:tts-provider') || 'edge'));
  const [ttsModel, setTtsModelState] = useState(() => localStorage.getItem('substrate:tts-model') || '');
  const [sttProvider, setSttProviderState] = useState<STTProvider>(() => {
    const saved = localStorage.getItem('substrate:stt-provider') as STTProvider | null;
    return saved === 'openai' ? 'openai' : 'local';
  });
  const [sttInputMode, setSttInputModeState] = useState<STTInputMode>(() => {
    const saved = localStorage.getItem('substrate:sttInputMode') as STTInputMode | null;
    return saved === 'browser' || saved === 'local' || saved === 'hybrid' ? saved : 'hybrid';
  });
  const [sttModel, setSttModelState] = useState(() => localStorage.getItem('substrate:stt-model') || 'base');
  const [wakeWordEnabled, setWakeWordEnabled] = useState(false);
  const [liveTranscriptionPreview, setLiveTranscriptionPreview] = useState(() => {
    const saved = localStorage.getItem('substrate:liveTranscriptionPreview');
    return saved === 'true'; // Default to disabled (fresh installs)
  });
  const [panelRatio, setPanelRatioState] = useState(() => {
    const saved = localStorage.getItem('substrate:panel-ratio');
    return saved ? Number(saved) : 75;
  });
  const [telemetryVisible, setTelemetryVisible] = useState(() => {
    const saved = localStorage.getItem('substrate:telemetry-visible');
    return saved !== 'false'; // Default to true (visible)
  });
  const [eventsVisible, setEventsVisible] = useState(() => {
    return localStorage.getItem('substrate:showEvents') === 'true'; // Default to false (hidden)
  });
  const [logVisible, setLogVisible] = useState(() => {
    return localStorage.getItem('substrate:showLog') === 'true'; // Default to false (hidden)
  });
  const [theme, setThemeState] = useState<ThemeName>(() => {
    const saved = localStorage.getItem('substrate:theme') as ThemeName | null;
  ...
  const [fontSize, setFontSizeState] = useState<number>(() => {
    const saved = localStorage.getItem('substrate:font-size');
    const parsed = saved ? parseInt(saved, 10) : NaN;
    return normalizeFontSize(parsed);
  });
  const [editorFontSize, setEditorFontSizeState] = useState<number>(() => {
    const saved = localStorage.getItem('substrate:editor-font-size');
    const parsed = saved ? parseInt(saved, 10) : NaN;
  ...
  const toggleSound = useCallback(() => {
    setSoundEnabled(prev => {
      const next = !prev;
      localStorage.setItem('substrate:sound', String(next));
      return next;
    });
  }, []);
  ...
  const toggleLiveTranscriptionPreview = useCallback(() => {
    setLiveTranscriptionPreview(prev => {
      const next = !prev;
      localStorage.setItem('substrate:liveTranscriptionPreview', String(next));
      return next;
    });
  }, []);
  ...
  const changeTtsProvider = useCallback((provider: TTSProvider) => {
    ...
    localStorage.setItem('substrate:tts-provider', provider);
    // Reset model when switching providers — models are provider-specific
    setTtsModelState('');
    localStorage.setItem('substrate:tts-model', '');
  }, []);

  const changeTtsModel = useCallback((model: string) => {
    setTtsModelState(model);
    localStorage.setItem('substrate:tts-model', model);
  }, []);
    ...
    fetch('/api/transcribe/config')
      ...
      .then(data => {
        ...
        if (serverModel && serverModel !== sttModel) {
          ...
          localStorage.setItem('substrate:stt-model', serverModel);
        }
  ...
  const changeSttProvider = useCallback((provider: STTProvider) => {
    ...
    localStorage.setItem('substrate:stt-provider', provider);
  ...
  const changeSttInputMode = useCallback((mode: STTInputMode) => {
    ...
    localStorage.setItem('substrate:sttInputMode', mode);
  }, []);

  const changeSttModel = useCallback((model: string) => {
    setSttModelState(model);
    localStorage.setItem('substrate:stt-model', model);
  ...
  const toggleTtsProvider = useCallback(() => {
    setTtsProvider(prev => {
      ...
      localStorage.setItem('substrate:tts-provider', next);
      return next;
    });
  }, []);
  ...
  const setPanelRatio = useCallback((ratio: number) => {
    ...
    localStorage.setItem('substrate:panel-ratio', String(ratio));
  }, []);

  const toggleTelemetry = useCallback(() => {
    setTelemetryVisible(prev => {
      const next = !prev;
      localStorage.setItem('substrate:telemetry-visible', String(next));
      return next;
    });
  }, []);
  ...
  const toggleEvents = useCallback(() => {
    setEventsVisible(prev => {
      const next = !prev;
      localStorage.setItem('substrate:showEvents', String(next));
      return next;
    });
  }, []);
  ...
  const toggleLog = useCallback(() => {
    setLogVisible(prev => {
      const next = !prev;
      localStorage.setItem('substrate:showLog', String(next));
      return next;
    });
  }, []);
  ...
  const setTheme = useCallback((newTheme: ThemeName) => {
    ...
    localStorage.setItem('substrate:theme', newTheme);
  }, []);

  const setFont = useCallback((newFont: FontName) => {
    setFontState(newFont);
    localStorage.setItem('substrate:font', newFont);
  }, []);
  ...
  const setEditorFontSize = useCallback((size: number) => {
    ...
    localStorage.setItem('substrate:editor-font-size', String(normalized));
  }, []);
