...
export const InputBar = forwardRef<InputBarHandle, InputBarProps>(function InputBar({ onSend, isGenerating, onWakeWordState, agentName = 'Agent' }, ref) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('Substrate:language-changed', fetchLang);
    window.addEventListener('Substrate:voice-phrases-changed', handlePhrasesChanged);
    return () => {
      window.removeEventListener('Substrate:language-changed', fetchLang);
      window.removeEventListener('Substrate:voice-phrases-changed', handlePhrasesChanged);
      currentController?.abort();
    };
  }, []);
