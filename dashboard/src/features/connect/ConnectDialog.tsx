...
// import SubstrateLogo from '@/components/SubstrateLogo';
...
export function ConnectDialog({
  ...
  const handleConnect = async () => {
    ...
    // For server-side auth, use the session token from login (not the dialog input).
    // For manual connections, use whatever the user typed.
    const effectiveUrl = (isOfficialUrl && officialUrl) ? officialUrl.trim() : url.trim();
    const effectiveToken = (serverSideAuth && isOfficialUrl) ? getSessionToken() : token.trim();
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogTitle className="mt-1 text-lg font-semibold tracking-[-0.03em] text-foreground sm:text-xl">
                  Connect to Substrate Gateway
                </DialogTitle>
              </div>
            </div>
            <DialogDescription className="max-w-[42ch] text-sm leading-6 text-muted-foreground">
              Point the dashboard at Substrate's gateway endpoint and bring the cockpit online.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 py-4 pb-[max(1.067rem,env(safe-area-inset-bottom))] sm:gap-5 sm:px-6 sm:py-6">
          <div className="hidden gap-4 sm:grid sm:grid-cols-2">
            <div className="shell-panel rounded-2xl px-4 py-3">
              ...
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                Talks to Substrate over WebSocket and keeps the session state in sync live.
              </p>
            </div>
            <div className="shell-panel rounded-2xl px-4 py-3">
          ...
          <div className="mt-auto flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="max-w-[34ch] text-xs leading-5 text-muted-foreground">
              Keep bound to localhost unless you explicitly want remote access.
            </p>
            <Button
