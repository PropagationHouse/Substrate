...
The Remote Bridge System allows you to access your Tiny Pirate agent from any device on your network, including mobile phones and tablets.

## Overview

The Remote Bridge System consists of several components:

1. **Remote Bridge** - A lightweight web server that provides a web interface to interact with your Tiny Pirate agent
2. **Command Server** - A server that receives commands from the Remote Bridge and forwards them to your main Tiny Pirate application
3. **Command Pipe** - A named pipe communication system that allows direct communication between the Command Server and your main Tiny Pirate application
4. **Response Server** - A server that receives responses from your main Tiny Pirate application and forwards them to the Remote Bridge
...
2. Integrate with your main Tiny Pirate application:
   - Copy `main_app_integration.py` to your main Tiny Pirate directory
   - Add the following code to your main application (`proxy_server.py`):
     ...
     ```
   - Restart your main Tiny Pirate application
...
The Remote Bridge provides a web interface that mimics the Tiny Pirate chat interface. It:
- Serves a web UI accessible from any device on your network
- Forwards commands to the Command Server
- Displays responses from your main Tiny Pirate application
- Provides status information about the system

### Command Server (`command_server.py`)

The Command Server receives commands from the Remote Bridge and forwards them to your main Tiny Pirate application. It:
- Listens for commands on port 8766
- Uses the Command Pipe to send commands to your main Tiny Pirate application
- Receives responses from your main Tiny Pirate application
- Forwards responses back to the Remote Bridge

### Command Pipe (`command_pipe.py`)

The Command Pipe provides direct communication between the Command Server and your main Tiny Pirate application using Windows named pipes. It:
- Creates a named pipe server in your main Tiny Pirate application
- Allows the Command Server to send commands directly to your main Tiny Pirate application
- Ensures commands are executed in the same process and context as your main app

### Response Server (`response_server.py`)

The Response Server receives responses from your main Tiny Pirate application and makes them available to the Remote Bridge. It:
...
The Response Notifier is integrated into your main Tiny Pirate application and sends responses to the Response Server. It:
- Intercepts responses sent to the frontend in your main Tiny Pirate application
...
2. **Commands not executing**
   - Make sure your main Tiny Pirate application is running
...
The Remote Bridge System is designed for use on trusted networks only. It does not include authentication or encryption. If you need to access your Tiny Pirate agent from untrusted networks, consider:
...
- Voice output is only available on the main Tiny Pirate application
