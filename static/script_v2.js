            ...
            <div class="calendar-day ${isToday ? 'today' : ''}" onclick="openDayModal('${dateStr}')">
                ...
                <div class="day-content">
                    ${dayItems.map(item => {
                        ...
                        return `<div class="content-pill compact" style="border-left-color:${statusColor};" onclick="event.stopPropagation(); openContentModal('${item.id}')">${item.title}</div>`;
                    }).join('')}
                </div>
            </div>
        `;
...
async function doResearchSearch() {
    ...
    try {
        ...
        // Also notify the parent (Substrate) about new research for sync
        try {
            window.parent.postMessage({ type: 'substrate:research-result', query, summary: researchState.lastSearchSummary, sources: researchState.searchSources }, '*');
        } catch(e) {}
...
function initModals() {
    ...
    // Mode toggle label update
    const brainstormModeToggle = document.getElementById('brainstormModeToggle');
    if (brainstormModeToggle) {
        brainstormModeToggle.addEventListener('change', () => {
            const label = document.getElementById('brainstormModeLabel');
            if (label) label.textContent = brainstormModeToggle.checked ? 'One-shot' : 'Chat';
        });
    }
...
async function brainstormChat() {
    ...
    const sendBtn = document.getElementById('brainstormSendBtn');
    const modeToggle = document.getElementById('brainstormModeToggle');
    const isOneShot = modeToggle && modeToggle.checked;
...
function renderDayView(container) {
    ...
    container.innerHTML = `
        <div class="planner-header">
            ...
            <button class="btn-ghost" style="font-size:0.7rem;padding:0.15rem 0.4rem;opacity:0.5;" onclick="renamePlanner()" title="Rename planner">✏️</button>
        ...
        ${scheduledItems.length > 0 ? `
            <div class="planner-section-label">Scheduled Content</div>
            ${scheduledItems.map(item => `
                <div class="planner-item scheduled" onclick="openContentModal('${item.id}')">
    ...
    container.innerHTML = `
        ...
        <div class="planner-week-grid">
            ${days.map(d => {
                ...
                return `
                    <div class="planner-week-day ${isToday ? 'today' : ''}" onclick="plannerState.selectedDate='${ds}'; plannerState.view='day'; renderDayPlanner();">
...
function openSubstrateResearch() {
    const query = document.getElementById('researchSearchInput')?.value?.trim() || '';
    // If embedded in Substrate dashboard iframe, message parent to open Research panel
    if (window.parent !== window) {
        window.parent.postMessage({ type: 'substrate:open-research', query }, '*');
    } else {
        // Standalone mode: open Substrate dashboard research in new tab
